...
class AutoencoderModel:
    ...
    def train(
        self, X_train: np.ndarray, epochs: int = 50, learning_rate: float = 0.001
    ):
        """
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0:
                print(
                    f"Autoencoder Epoch [{epoch+1}/{epochs}], Loss: {loss.item():.4f}"
                )
...
class HybridModel:
    ...
    def _normalize_with_bounds(
        values: np.ndarray, bounds: Tuple[float, float], invert: bool = False
    ) -> np.ndarray:
    ...
    def calculate_hybrid_score(
        ...
        if self.iso_score_bounds is not None:
            iso_normalized = self._normalize_with_bounds(
                iso_scores, self.iso_score_bounds, invert=True
            )
        else:
            if len(iso_scores) > 1:
                iso_normalized = (
                    1
                    - MinMaxScaler().fit_transform(iso_scores.reshape(-1, 1)).flatten()
                )
            else:
        ...
        if self.ae_error_bounds is not None:
            ae_normalized = self._normalize_with_bounds(
                ae_errors, self.ae_error_bounds, invert=False
            )
        else:
            if len(ae_errors) > 1:
                ae_normalized = (
                    MinMaxScaler().fit_transform(ae_errors.reshape(-1, 1)).flatten()
                )
            else:
        ...
        hybrid_scores = (
            0.4 * iso_normalized + 0.4 * ae_normalized + 0.2 * dbscan_normalized
        )
    ...
    def predict(
        self, X: np.ndarray, threshold: float = 0.75
...
def load_models(filepath: str = "trained_models") -> Tuple[HybridModel, object]:
    ...
    hybrid_model.iso_forest = joblib.load(
    ...
    if os.path.exists(calibration_path):
        ...
        hybrid_model.default_threshold = float(
            calibration.get("default_threshold", hybrid_model.default_threshold)
        )
